...
**`grep`**<sup>[3]</sup> gets its name from the `ed` command `g/re/p` (**G**lobally search a **R**egular **E**xpression and **P**rint). It is a utility for searching fixed-strings or regular expressions in plain text files. The basic syntax of a grep command requires two arguments:
